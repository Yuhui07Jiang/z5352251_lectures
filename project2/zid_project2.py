...
def read_prc_csv(tic):
    ...
    """
    # <COMPLETE THIS PART>
    csv_file = os.path.join(cfg.DATADIR, f"{tic.lower()}_prc.csv")
...
def mk_prc_df(tickers, prc_col='adj_close'):
    ...
    **not** necessarily represent the actual contents of the data frame you
    ...
    for ticker in tickers:
        ...
        prc_data.append(df[[prc_col]].rename(columns={prc_col: ticker.lower()}))

    combined_df = pd.concat(prc_data, axis=1)
    return combined_df
...
def mk_ret_df(prc_df):
    ...
    """
    # <COMPLETE THIS PART>
    mkt_df = pd.read_csv(cfg.FF_CSV,index_col=0)
    mkt_df.index = pd.DatetimeIndex(mkt_df.index)
    ret_df = prc_df.pct_change()
    ret_df=ret_df.join(mkt_df['mkt'])
    return ret_df
...
def mk_aret_df(ret_df):
    ...
    """
    # <COMPLETE THIS PART>
    aret_df = ret_df.drop(columns=['mkt']).subtract(ret_df['mkt'], axis=0)
    return aret_df
...
def get_avg(df, col, year):
    ...
    """
    #<COMPLETE THIS PART>
    years= df.index.year == year
    col_values = df.loc[years, col].dropna()
    avg = col_values.mean()
    return avg
...
def get_ann_ret(ser, start, end):
    ...
    """
    # <COMPLETE THIS PART>
    x=ser.loc[start:end].dropna()
    N=len(x)
    total_ret=x.prod()
    annual_ret=(total_ret)*(252/N) - 1
    return annual_ret
