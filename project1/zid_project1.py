...
def get_tics(pth):
    ...
    """
    with open(pth) as tickers:
        tics = {}
        for word in tickers:
            tics[word.split('"')[3].lower()] = word.split('"')[1].lower()
    return tics
...
def read_dat(tic):
    ...
    tic_path = os.path.join(DATDIR, f"{tic}.prc.dat")
    with open(tic_path) as tickers:
        word = []
        for word in tickers:
...
def line_to_dict(line):
    ...
    position_start = 0
    for i in COLUMNS:
        position_end = position_start + COLWIDTHS[i]
        dictionary[i] = word[position_start:position_end]
